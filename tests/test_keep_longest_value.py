...
def test_keep_longest():
	...
	res = merge.on_slk_intervals(
		...
		[
			...
			merge.Action('measure', rename="measure longest value", aggregation=merge.Aggregation.KeepLongestValue()),
			merge.Action('category', rename="category longest segment", aggregation=merge.Aggregation.KeepLongestSegment()),
			merge.Action('category', rename="category longest value", aggregation=merge.Aggregation.KeepLongestValue()),
		]
	)
